...
const CreateDataSource = ({ open, onClose }: CreateDataSourceProps) => {
  ...
  useEffect(() => {
    if (dataLoaders && dataLoaders.length && !selectedDataSourceType) {
      setSelectedDataSourceType(dataLoaders[0].type)
    }
  }, [dataLoaders, selectedDataSourceType])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      notify(
        'success',
        'Data Source is successfully created!',
        'Updated data sources will be available to use after 3-5 minutes.'
      )
    } catch (err: any) {
      notify(
        'error',
        ...
        err?.error ||
          err?.details?.msg ||
          err?.message ||
          'There was an error while creating the data source.'
      )
    }
    setIsSaving(false)
  }
      ...
      <>
        ...
        <div className="h-[calc(100vh-124px)] overflow-y-auto p-4">
          ...
          <div>
            <div className="mb-2">
              <label>
                <div className="label-text font-lab mb-1">Data Source Type</div>
                <Select
                  id="data_sources"
