...
from backend.modules.query_controllers.default.types import DefaultQueryInput, RerankerQuery, DEFAULT_QUERY, DEFAULT_RERANK_QUERY
...
class DefaultQueryController:
    ...
    async def answer(self, request: DefaultQueryInput = Body(DEFAULT_QUERY)):
        ...
        try:
            ...
            elif request.model_configuration.provider == "truefoundry":
                # Get the LLM
                llm = TrueFoundryChat(
                    model=request.model_configuration.name,
    ...
    @post("/answer-with-reranking")
    async def answer_with_reranking(self, request: RerankerQuery = Body(DEFAULT_RERANK_QUERY)):
        """
        Sample answer method to answer the question using the context from the collection
        """
        try:
