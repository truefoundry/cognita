...
class ModelGateway:
        ...
        with open(MODELS_CONFIG_PATH) as f:
            ...
            for provider_config in self.provider_configs:
                if provider_config.api_key_env_var and not os.environ.get(
                    ...
                    raise ValueError(
                        f"Environment variable {provider_config.api_key_env_var} not set. Cannot initialize the model gateway."
                    )
                ...
                for model_id in provider_config.embedding_model_ids:
                    ...
                    self.embedding_models.append(
                        ModelConfig(name=model_name, type=ModelType.embedding)
                    )
                ...
                for model_id in provider_config.llm_model_ids:
                    ...
                    self.llm_models.append(
                        ModelConfig(
                            name=model_name, type=ModelType.chat, parameters=parameters
                        )
                    )
    ...
    def get_embedder_from_model_config(self, model_name: str) -> Embeddings:
        ...
        if model_provider_config.provider_name == "sambastudio":
            ...
            api_key = (
                os.environ.get(model_provider_config.api_key_env_var, "")
                if model_provider_config.api_key_env_var
                else "EMPTY"
            )
    ...
    def get_llm_from_model_config(
        ...
        if model_provider_config.provider_name in [
            "openai",
            "local-ollama",
            "local-infinity",
            "truefoundry",
            ...
            raise ValueError(
                f"Unsupported LLM provider: {model_provider_config.provider_name}"
            )
